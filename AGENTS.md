...
- 다음 커밋부터 파일 수가 아니라 논리적·원자적 작업 단위로 커밋합니다. 기존 커밋은 별도 요청 없이 amend·rebase·squash하지 않습니다.
- 하나의 기능·수정·마이그레이션을 완성하는 파일은 함께 커밋하고 관련 없는 변경은 분리합니다. 한 파일의 독립 변경도 가능하면 hunk 단위로 분리합니다.
- 각 커밋은 가능하면 독립적으로 cherry-pick·revert·bisect할 수 있고 컴파일 가능해야 합니다. 분리 시 빌드가 깨지는 상호 의존 변경은 함께 묶습니다.
- Unity 에셋과 대응 `.meta`는 생성·수정·삭제·이름 변경을 항상 같은 커밋에 포함합니다.
- 서드파티 패키지의 동일 업데이트·설치·통합 구성요소는 함께 묶고, 생성 코드와 변경을 유발한 생성 설정 또는 원본 스키마도 함께 묶습니다.
- `git add .`와 `git add -A`를 금지합니다. 파일 경로나 hunk를 명시해 스테이징하고 `git diff --cached`로 범위를 검증합니다.
