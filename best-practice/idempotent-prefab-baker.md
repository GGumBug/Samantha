...
### 2-2. 강제를 그만두려면 반대 방향을 적는다 (2026-09-16)

`ForceDisabled(reaction)` 호출을 **지우기만 하면 아무것도 되돌아가지 않는다**. 프리팹에 `m_Enabled: 0`
오버라이드가 이미 구워져 있고, §2의 "안 건드린 것은 그대로 둔다"가 그 값을 보존하기 때문이다.
되돌리려면 `ForceEnabled(reaction)`을 **명시적으로** 부른다.

일반형: **강제를 제거하는 것과 반대로 강제하는 것은 다르다.** 이미 구워진 산출물이 있으면 전자는
아무 일도 하지 않는다. 수렴형 도구(마이그레이션·설정 관리·코드 생성)가 모두 같은 성질을 갖는다.
합격 기준을 "호출을 지웠다"가 아니라 **산출물의 값**으로 적어야 이 차이가 보인다.

되돌린 뒤 **오버라이드 항목은 사라지지 않고 값만 0에서 1로 바뀐다.** 그 편이 더 안전한 상태다.
항목이 사라지면 중첩 자산 쪽이 꺼지는 날 소비자도 조용히 따라 꺼진다(§6 순서 계약과 같은 계급).
...
- [ ] 강제를 그만두는 변경이면 호출을 지우는 대신 **반대 방향을 명시**했는가 (§2-2)
