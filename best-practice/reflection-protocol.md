...
Stop 이벤트에서 git 변경을 감지하여 필요 시 `/reflect` 사용을 제안. 자동 반영 X, 제안만.
...
| 안티패턴 경고 | `best-practice/solid-unity-principles.md` |
| 에이전트 기본 행동 개선 | `.claude/agents/<name>.md` |
| 위임 전략 업데이트 | `.claude/rules/unity-delegation.md` |
| 반복 리팩토링 교훈 | `best-practice/refactoring-lessons.md` |
| 상태 머신/생명주기 교훈 | `best-practice/node-lifecycle-patterns.md` |
...
이번 세션에서 적용된 반영 예시:
- Pre-Implementation Gate → `sonny.md` / `jarvis.md` 기본 행동 섹션
- Deferred Commit State Machine → `best-practice/node-lifecycle-patterns.md` 신규
- SOLID·하드코딩 카탈로그 → `best-practice/solid-unity-principles.md` 신규
- Sonny `maxTurns` 절단 감지 → `unity-delegation.md` 섹션 추가
- 3가지 세션 격리 전략 비교 (향후 반영 후보)
