...
3. 의존성 prep 단계 분리 (병렬 위임 전 — 아래 §병렬 위임 prep 참고)
4. 적절한 팀원에게 Agent 도구로 위임 (병렬 가능)
5. 결과 수신 → 품질 검증 → 통합
6. 사용자에게 결과 보고 및 다음 단계 제안
```

## 병렬 위임 prep — 공유 자원 충돌 방지 (필수)

병렬 에이전트는 **같은 파일에 동시 쓰기 → 충돌**한다. 위임 전 prep 단계로 분리한다.

### 충돌 자원 (병렬 금지 — 단일 prep 에이전트가 먼저 처리)

- `package.json` / `package-lock.json` / `pnpm-lock.yaml` — npm/pnpm install 동시 실행 금지
- `tsconfig.json` / `next.config.*` / `tailwind.config.*` — 설정 파일 동시 편집 금지
- 같은 파일 내 동일 함수/컴포넌트 — 두 에이전트가 같은 파일 같은 영역 수정 금지

### prep 분리 패턴

```
1차 (단일 에이전트): 의존성 설치, 공유 설정 변경, 타입/스키마 정의
2차 (병렬 N 에이전트): 1차 결과를 전제로 독립 영역 작업
```

위임 시 각 에이전트에게 **수정 가능 파일 범위를 명시**한다. "Friday는 app/foo/, HAL은 app/api/foo/" 처럼 경로 격리. 같은 lock 파일을 건드리는 작업이면 prep 에이전트가 먼저 install 후 commit, 그 다음 병렬 위임.

### 우회 금지

- 합리화 차단: "이번엔 같은 lock 만져도 운 좋을 것" — 병렬 install 충돌은 100% 재현. 운 의존 금지
- 정공법: prep 분리 또는 순차 위임. "병렬이 빠르니까" 이유로 충돌 가능 위임 금지
