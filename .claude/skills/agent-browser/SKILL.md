---
name: agent-browser
description: AI 에이전트를 위한 브라우저 자동화 CLI. 사용자가 웹사이트와 상호작용해야 할 때 사용한다. 페이지 탐색, 폼 작성, 버튼 클릭, 스크린샷 촬영, 데이터 추출, 웹 앱 테스트, 또는 브라우저 작업 자동화 등에 활용한다. "웹사이트 열기", "폼 입력", "버튼 클릭", "스크린샷 찍기", "페이지에서 데이터 스크래핑", "웹 앱 테스트", "사이트 로그인", "브라우저 동작 자동화" 등 프로그래밍 방식의 웹 상호작용이 필요한 모든 작업에 사용한다.
...
**실제 기기:** 사전 설정된 경우 물리적 iOS 기기에서도 작동한다. `xcrun xctrace list devices`에서 얻은 UDID를 사용해 `--device "<UDID>"`를 지정한다.

## 참조(Ref) 수명 주기 (중요)

참조(`@e1`, `@e2` 등)는 페이지가 변경되면 무효화된다. 다음 상황 뒤에는 반드시 재스냅샷 한다:
...
참조를 사용할 수 없거나 신뢰할 수 없으면 시맨틱 로케이터를 사용한다:
...
| 참고 자료 | 사용 시점 |
|-----------|-----------|
| [references/commands.md](references/commands.md) | 모든 옵션이 포함된 전체 명령어 참고 |
| [references/snapshot-refs.md](references/snapshot-refs.md) | 참조 수명 주기, 무효화 규칙, 문제 해결 |
| [references/session-management.md](references/session-management.md) | 병렬 세션, 상태 저장, 동시 스크래핑 |
| [references/authentication.md](references/authentication.md) | 로그인 흐름, OAuth, 2FA 처리, 상태 재사용 |
| [references/video-recording.md](references/video-recording.md) | 디버깅 및 문서화를 위한 녹화 워크플로우 |
| [references/proxy-support.md](references/proxy-support.md) | 프록시 설정, 지역 테스트, 순환 프록시 |

## 바로 사용 가능한 템플릿

| 템플릿 | 설명 |
|--------|------|
| [templates/form-automation.sh](templates/form-automation.sh) | 검증이 포함된 폼 자동 입력 |
| [templates/authenticated-session.sh](templates/authenticated-session.sh) | 한 번 로그인 후 상태 재사용 |
