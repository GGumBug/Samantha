...
> `C:\Unity_Projects\Double-Down` 전용 지침. 실체는 Samantha 저장소가 관리하고(git 추적), Double-Down 루트의 `CLAUDE.md`가 `.claude/` 정션을 통해 import한다. **Double-Down 안에서 이 파일을 수정하면 Samantha 워킹트리가 변경된다** — 정상 동작이며, 커밋은 Samantha 저장소에서 한다.

팀 위임 규칙·SOLID/SSOT 헌법·UniTask 룰·검증 정책은 `.claude/rules/*.md`가 파일 종류에 따라 자동 주입하므로 여기 복사하지 않는다.
...
의존은 **항상 안쪽으로만** 흐릅니다. asmdef `references`가 이 계약을 컴파일 타임에 강제한다.
...
- **Domain에 `using UnityEngine` 금지** — `noEngineReferences: true`라 컴파일 자체가 실패한다. `Vector2`/`Random`/`Time`/`Debug.Log`가 필요하면 설계가 틀린 것이다. 이 순수성 덕분에 `Tools/HeadlessSim`이 Unity 없이 Domain을 돌린다. 깨뜨리면 헤드리스 시뮬레이션이 죽는다.
- **Presentation → Domain 직접 참조 금지** — Application의 `BoardSnapshot`/`PresentationVocabulary`를 경유한다. 뷰가 도메인 타입을 직접 읽으면 경계가 무너집니다.
- **DI 조립은 Boot에서만** — Reflex 컨테이너 등록이 다른 계층으로 새면 Composition Root가 아니다.
- 새 어셈블리 참조를 추가하기 전에 **위 표의 방향을 위반하는지 먼저 확인**하고, 위반이면 코드를 쓰기 전에 사용자에게 구조 대안을 제시한다.
...
GameCore 수정은 Double-Down 외 프로젝트에도 영향을 주는 **공용 변경**이다. 게임 고유 로직을 GameCore에 넣지 말고, 반대로 GameCore 시그니처 변경 시 영향 범위를 명시한다.
...
**모드 판정은 "동기/POCO 여부"가 아니라 내부에서 쓰는 Unity API 기준**이다. `Object.Destroy` / `DontDestroyOnLoad` / 씬 API를 쓰면 PlayMode. 판정 전 grep 선행 의무 ([best-practice/unity-test-mode-selection.md](../../best-practice/unity-test-mode-selection.md)).

Domain 로직은 `Tools/HeadlessSim`(.NET 콘솔)으로 Unity 없이 대량 시뮬레이션할 수 있다. 밸런싱·확률 검증은 Editor 재생보다 이쪽이 빠릅니다.

## 데이터 파이프라인

BGDatabase(`Assets/BansheeGz`) → 생성 코드 `DoubleDown.Data.Generated` → Infrastructure에서 소비.

**생성 코드와 그 변경을 유발한 생성 설정·원본 스키마는 같은 커밋**에 포함한다. 생성물만 커밋하면 재생성이 불가능해집니다.

## 워크플로 — 카드 단위

작업은 **카드 번호** 단위로 진행한다 (현재 26.5까지 완료). 커밋 메시지는 저장소 형식을 따릅니다:
...
- **AI 귀속 표시 절대 금지** — `Co-Authored-By`, "Generated with Claude Code" 등 어떤 형태도 넣지 않는다
- `git add .` / `-A` 금지 — 경로 명시 후 `git diff --cached`로 범위 검증
- 커밋은 `/commit` 스킬 사용 권장 (저장소 규칙 내장, 격리 컨텍스트에서 실행)
- **신규 `.cs`의 `.meta` 짝 확인** — 에이전트가 만든 `.cs`의 `.meta`는 사용자가 Unity를 연 뒤 생성된다
