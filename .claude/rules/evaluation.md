...
5. **주입 원문은 손으로 적지 않고 구현 파일에서 읽어 만든다**: 손으로 적은 `\uXXXX` 이스케이프는 틀려도 조용하다. 치환 스크립트는 **치환 건수를 출력하고 0건이면 실패로 끝낸다**. 0건은 뮤테이션 없이 초록이라서 검증했다는 착각이 된다.
   (2026-09-22 `잃`(U+C798)을 `있`(U+C788)로 적어 치환 0회. 스크립트가 건수를 출력해 잡았다)
...
**살아남은 뮤테이션은 시험 부족이 아니라 근거 오류 신호다.** 시험이 배포 에셋을 불러오지 않으면 설정 축이 통째로 빠지고, 장식용 여백이 기하 결함을 가린다. 상세: [best-practice/coincidental-pass.md](../../best-practice/coincidental-pass.md)
