...
## ⚠️ Gotchas — Claude가 자주 빠지는 게임 디자인 함정 (반드시 숙지)

> 기획안 작성 및 조율 시 주의하세요.

### G1. 과도한 스코프(Over-scoping) 제안
❌ "오픈월드", "수백 개의 스킬 조합", "AI 반역자 시스템" 등 인디/소규모 팀에 맞지 않는 거대한 기능 기획
✅ 제한된 시스템으로 깊이를 파낼 수 있는 코어 루프에 집중해야 합니다.

### G2. 구체적인 수치 부재 (너무 모호한 계약)
❌ `movementSpeed` = "알맞게 빠른 속도" 와 같이 프로그래머(Sonny)가 구현할 수 없는 모호한 기획
✅ 반드시 기준 수치 (예: `5.0 m/s`, `시간 180초`) 등 명확한 기본값을 계약 데이터로 제공해야 합니다.

### G3. UI/UX 요소 기획 누락
❌ 게임 플레이 메카닉만 기획하고 메뉴, 설정 창, 종료, 튜토리얼 팝업 등 필수 흐름 간과
✅ Rachael(UI/UX)이 작업할 수 있도록 앞선 '6. UI 스펙'에 전체 스크린과 흐름을 포함해야 합니다.
